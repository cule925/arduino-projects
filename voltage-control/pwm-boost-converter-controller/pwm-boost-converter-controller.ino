...
/* Timer 1 ICR1 register value top value substraction */
#define ICR1_TOP_REG_VAL_LIMIT_PERCENTAGE (90)

/* Timer 1 ICR1 max register value limit.
 * In case of the boost converter, the switch must not
 * be open 100 % of the timer period (100 % of duty cycle)
 * if target boost voltage out is unreacheable to prevent
 * coil and wire damage.
 */
#define ICR1_TOP_REG_VAL_LIMIT ((ICR1_TOP_REG_VAL_LIMIT_PERCENTAGE * ICR1_TOP_REG_VAL) / 100)

/* Target output boost voltage (millivolts) */
#define V_OUT_BOOST (5000U)

/* Boost output base (millivolts) */
#define V_OUT_BOOST_BASE (12000U)

/* Boost output max */
#define V_OUT_BOOST_MAX (24000U)
...
void loop()
{
  ...
  uint32_t voltage_boost_out_diff = 0;

  // Read both analog pins A0 and A1
  adc_0_result = analog_read(0);
  adc_1_result = analog_read(1);

  // Calculate target voltage increase from base using pin A1
  voltage_boost_out_diff = (V_OUT_BOOST_MAX - V_OUT_BOOST_BASE) * (uint32_t)adc_1_result / 1023;

  // Calculate target analog value that should be on pin A0 using the target voltage
  r2_voltage = (voltage_boost_out_diff + V_OUT_BOOST_BASE) * r2_resistance / (r1_resistance + r2_resistance);
  adc_target_value = 1023 * r2_voltage / V_ADC_MAX;

  // If too little voltage, increase duty cycle on MOSFET (decrease on BJT)
  if (adc_0_result < adc_target_value && ocr1a_reg_value < ICR1_TOP_REG_VAL_LIMIT) ocr1a_reg_value++;
  ...
  Serial.print(voltage_boost_out_diff + V_OUT_BOOST_BASE);
}
...
void setup_pwm()
{
  ...
  // Set initial output compare register A value
  OCR1A = 0;
}
